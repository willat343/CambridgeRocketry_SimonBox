...
public abstract class RingComponent extends StructuralComponent implements Coaxial {
	...
	public RockPartsData getRockPartsData(Simulation inputSimulation) {
		...
		double thisInnerRadius = this.getInnerRadius();
		double thisOuterRadius = this.getOuterRadius();
		// note: average radius = inner + outer radius / 2
		// diameter = radius * 2, therefore, it's equal to inner+outer
		double thisDiameter = (thisOuterRadius + thisInnerRadius);
		double thisMass = this.getMass();
		double thisPosition = this.getAbsolutePosition();
		String thisName = this.getName();
		double thisRadialPosition = this.getRadialPosition();
		
		// create camrocksim component
		CylinderData thisCylinderData = new CylinderData(
				thisLength, thisDiameter, thisMass,
				thisPosition, thisRadialPosition, thisName);
		
		// add component to main stage
		return (RockPartsData) thisCylinderData;
