...
public class NoseCone extends Transition {
	...
	public RockPartsData getRockPartsData(Simulation thisSimulation) {
		// NoseCone to NoseConeData
		
		// extract data
		double thisLength = this.getLength();
		double thisDiameter = this.getAftRadius() * 2;
		double thisPosition = this.getAbsolutePosition();
		double thisMass = this.getMass();
		int thisShapeID = this.getShapeID();
		String thisName = this.getName();
		
		// create camrocksim nose cone
		NoseConeData thisNoseConeData = new NoseConeData(
				thisLength, thisDiameter, thisPosition, thisMass,
				thisShapeID, thisName);
		
		// return RockPartsData
		return (RockPartsData) thisNoseConeData;
	}
