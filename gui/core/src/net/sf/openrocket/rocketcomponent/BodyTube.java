...
public class BodyTube extends SymmetricComponent implements MotorMount, Coaxial {
	...
	public RockPartsData getRockPartsData(Simulation inputSimulation) {
		...
		double thisID = this.getInnerRadius() * 2;
		double thisOD = this.getOuterRadius() * 2;
		double thisMass = this.getMass();
		double thisPosition = this.getAbsolutePosition();
		String thisName = this.getName();
		
		// create camrocksim component
		BodyTubeData thisBodyTubeData = new BodyTubeData(
				thisLength, thisID, thisOD, thisMass, thisPosition, thisName);
		
		// add component to main stage
		return (RockPartsData) thisBodyTubeData;
