...
public abstract class FinSet extends ExternalComponent {
	...
	public double getBodyDiameter() {
		// get parent to ask for bodydiameter
		RocketComponent thisParent = this.getParent();
		
		double thisBodyDiameter = 0.0;
		
		// only overwrite if it's a bodytube.. (other components do not have a diameter)
		if (thisParent instanceof BodyTube) {
			// is a bodytube
			BodyTube thisBodyTube = (BodyTube) thisParent;
			
			thisBodyDiameter = thisBodyTube.getOuterRadius() * 2;
		}
		
		return thisBodyDiameter;
	}
	
	
	@Override
	public RockPartsData getRockPartsData(Simulation inputSimulation) {
		// return RockPartsData conversion
		
		int thisNumberOfFins = this.getFinCount();
		double thisRootChord = this.getLength();
		double thisTipChord = this.getLength();
		double thisSpan = this.getSpan();
		double thisSweep = 0.0;
		double thisThickness = this.getThickness();
		double thisBodyDiameter = getBodyDiameter();
		double thisMass = this.getMass();
		double thisPosition = this.getAbsolutePosition();
		String thisName = this.getName();
		
		FinsetData thisFinSetData = new FinsetData(
				thisNumberOfFins, thisRootChord, thisTipChord, thisSpan,
				thisSweep, thisThickness, thisBodyDiameter, thisMass, thisPosition, thisName);
		
		// return component
		return (RockPartsData) thisFinSetData;
		
	}
	
	@Override
	public final double getAbsolutePosition() {
		// absolute position wrt nosecone
