...
public class Transition extends SymmetricComponent {
	...
	public int getShapeID() {
		/* returns a shapeID for camrocksim
		* 0 ogive (and all others)
		* 1 cone
		* 2 parabolic
		*/
		
		int ShapeID = 0; // default ogive
		
		if (this.type == Shape.CONICAL) {
			ShapeID = 1;
		} else if (this.type == Shape.PARABOLIC) {
			ShapeID = 2;
		}
		
		return ShapeID; // default ogive
	...
	public RockPartsData getRockPartsData(Simulation thisSimulation) {
		// Transition to ConicTransData
		
		// extract data
		double thisLength = this.getLength();
		double thisUpstreamDiameter = this.getForeRadius() * 2;
		double thisDownstreamDiameter = this.getAftRadius() * 2;
		double thisThickness = this.getThickness();
		double thisMass = this.getMass();
		double thisPosition = this.getAbsolutePosition();
		String thisName = this.getName();
		
		// create camrocksim transition
		ConicTransData thisConicTransData = new ConicTransData(
				thisLength, thisUpstreamDiameter, thisDownstreamDiameter,
				thisThickness, thisMass, thisPosition, thisName);
		
		// return RockPartsData
		return (RockPartsData) thisConicTransData;
	}
